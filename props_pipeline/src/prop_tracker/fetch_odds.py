...
from .types import Event, PropOutcome
...
class TheOddsAPIProvider:
        ...
        for item in data or []:
            ...
            events.append(
                Event(
                    ...
                    sport=sport,
                    league=str(item.get("sport_title") or sport).upper(),
    ...
    def fetch_props(
        ...
        bookmakers_data = data.get("bookmakers", []) if isinstance(data, dict) else []
                ...
                for outcome in market.get("outcomes", []) or []:
                    ...
                    outcomes.append(
                        PropOutcome(
                            ...
                            extra={"book_title": book.get("title")},
                        )
                    )
...
def get_odds_provider(settings: Settings):
    provider = settings.odds_provider
    if provider == "theoddsapi":
        return TheOddsAPIProvider(settings)
    raise RuntimeError(f"Unsupported ODDS_PROVIDER: {provider}")

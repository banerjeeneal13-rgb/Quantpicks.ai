...
from nba_etl.cache import cache_path, read_cache, write_cache
...
def test_cache_ttl_forever(tmp_path: Path):
    url = "https://example.com"
    params = {"c": 3}
    cache_file = cache_path(tmp_path, url, params)
    payload = {"fetched_at": time.time() - 10000, "data": {"y": 2}}
    cache_file.write_text(json.dumps(payload), encoding="utf-8")
    loaded, fresh = read_cache(cache_file, ttl_seconds=0)
    assert loaded == {"y": 2}
    assert fresh is True
